#> sort_items:_impl/sort_by_count/descend/shift
#@within function sort_items:_impl/sort_by_count/descend/loop
...
data remove storage : _[-1].ascend[-1]
execute if score $1 _sort_items >= $2 _sort_items if data storage : _[-1].ascend[-1][-1] run function sort_items:_impl/sort_by_count/descend/move_rest
