#> sort_items:_impl/sort_by_count/_impl/ascend/shift
#@within function sort_items:_impl/sort_by_count/_impl/ascend/loop
...
data remove storage : _[-1].descend[-1]
execute if score $1 _sort_items <= $2 _sort_items if data storage : _[-1].descend[-1][-1] run function sort_items:_impl/sort_by_count/_impl/ascend/move_rest
